...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, norm='bnorm', learning_type='plain'):
        ...
        self.unpool1 = nn.ConvTranspose2d(in_channels=1*nker, out_channels=1*nker, kernel_size=2, stride=2, padding=0, bias=True)
...
class Hourglass(nn.Module):
    def __init__(self, in_channels, out_channels, nker, norm='bnorm', learning_type='plain'):
        ...
        self.dec1_2 = CBR2d(in_channels=1*nker, out_channels=1*nker, norm=norm)
        self.dec1_1 = CBR2d(in_channels=1*nker, out_channels= 1*nker, norm=norm)
...
class ResNet(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, learning_type='plain', norm = 'bnorm', nblk=16):
        ...
        res = []
        self.enc = CBR2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1, bias=True, norm=None, relu=0.0)
        for i in range(nblk):
            res += [ResBlock(nker, nker, kernel_size=3, stride=1, padding=1, bias=True, norm=norm, relu=0.0)]
        self.res = nn.Sequential(*res)
